...
def napravi_matricu():
    ...
    try:
        r = int(br_redovi.get())
        c = int(br_stupci.get())
        if r <= 0 or c <= 0 or r > 10 or c > 10:
            raise ValueError
    except:
         # Prikaz errora ako korisnik ne unese broj ili unese <=0 ili >10
         error = Label(aplikacija,text="REDOVI I STUPCI MORAJU BITI [1-10]!!!", 
                       bg="#65ba9e", fg="#E40F0F", font=("Arial", 20, "bold underline"))
         error.pack(pady=200)
    ...
    potvrdi = Button(matrica_frame, text="POTVRDI", font=("Arial", 20, "bold"), 
                      ...
                      command=potvrdi_matricu, bd=6)
    potvrdi.grid(row=r + 2, column=0, columnspan=c, pady=20)

def potvrdi_matricu():
    global m1, LABEL, potvrdi
    try:
    ...
    # Očisti stare odabir frame-ove, ali ne dira matrica_frame ni labele unosa
    for widget in aplikacija.winfo_children():
        if isinstance(widget, Frame) and widget != matrica_frame:
            widget.destroy()

    odabir = Frame(aplikacija, bg="#aa90bb")
    odabir.pack(pady=20)

    zbrajanje = Button(odabir, text="ZBRAJANJE", font=("Arial", 12, "bold"), 
                       bg="#946aaf", cursor="hand2", relief="groove", bd=6)
    zbrajanje.pack(side=LEFT, padx=10)

    oduzimanje = Button(odabir, text="ODUZIMANJE", font=("Arial", 12, "bold"), 
                        bg="#946aaf", cursor="hand2", relief="groove", bd=6)
